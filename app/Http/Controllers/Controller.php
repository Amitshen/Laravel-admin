...
class Controller extends BaseController
{
    ...
    public function profileImage($file)
    {
        ...
        // Public Folder
        $file->move(public_path('profile-image'), $imageName);
        // $file->storeAs($path, $imageName);
        return $imageName; 
    }

    public function featureImage($file)
    {
        # code...    
        $filename = $file->getClientOriginalName();
        $filename = pathinfo($filename, PATHINFO_FILENAME);
        $imageName = time().uniqid().str_replace(' ','-',$filename).'.'.$file->extension();
        // Public Folder
        $file->move(public_path('feature-image'), $imageName);
        // $file->storeAs($path, $imageName);
        return $imageName; 
    }
    ...
    public function UpdateImage($document ,$path)
    {
        $file = $document;
