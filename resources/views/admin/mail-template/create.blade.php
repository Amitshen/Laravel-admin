...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("dashboard.mails-template.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group mb-4 {{ $errors->has('message') ? 'has-error' : '' }}">
                ...
                <textarea id="message" name="message" placeholder="{{ __('Message') }}" class="form-control ckeditor">{{ old('message', isset($MailTemplate) ? $MailTemplate->message : '') }}</textarea>
